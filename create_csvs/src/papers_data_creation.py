...
def papers_data_creation(dataIn, dataOut):
    """
    Save in a csv file a processed data frame with only one paper per
    instance
    ...
    """
    data = read_csv(dataIn, sep=';')
    columns = [
        'autor_docente', 'autor_nao_docente', 'autores', 'id_producao', 'ano',
        'sub_tipo_producao', 'citescore', 'quartil_citescore', 'percentile',
        'quartil_percentile', 'qualis_estrato', 'PROGRAMAS',
        'progrmas_quantidade'
    ]
    ...
    for idPub in idPubs:
        ...
        row = DataFrame({
            ...
            'qualis_estrato': [qualisEst.get(prod.qualis_estrato.values[0])],
            'PROGRAMAS': [str(progs).replace("'", '').replace(', ', '-')],
...
def run(dataIn, dataOut):
    print("called Papers Data Creation")
    papers_data_creation(dataIn, dataOut)

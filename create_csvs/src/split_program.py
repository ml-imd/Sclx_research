...
def split_programs(dataIn, dataOut):
    """
    ...
    for id, row in data.iterrows():
        ...
        if length_programs == 1:
            ...
            split_programs[length_programs -
                           1] = split_programs[length_programs - 1][:-1]
            ...
            while len(split_programs) > 0:
                aux = DataFrame({
                    ...
                    'qualis': [row['qualis']],
                    'programas':
                    [str([split_programs.pop()]).replace("'", "")],
                    'prog._qtd': [1],
                    'Cluster': [row['Cluster']]
                })
...
def run(dataIn, dataOut):
    print("called Split Program")
    split_programs(dataIn, dataOut)
